...
mysocket_status mysocket::init()
{
  ...
  return(iResult==0 ? mysocket_status::SOCK_OK : mysocket_status::SOCK_ERROR);
}
...
inline static void myclosesocket(SOCKET x)
{
  closesocket(x);
}
...
mysocket_status tcp_mysocket::read(byte* dado, size_t len, long milisec) const
{
  if (!connected() || len<=0)
  {
    return(mysocket_status::SOCK_ERROR);
...
/// - mysocket_status::SOCK_OK, em caso de sucesso;
/// - mysocket_status::SOCK_ERROR, em caso de erro
mysocket_status tcp_mysocket::write(const byte* dado, size_t len) const
{
  if (!connected()) {
    return(mysocket_status::SOCK_ERROR);
  }
...
{
  return read((byte*)&num,sizeof(num),milisec); // sizeof(num) == 4
}
...
{
  return write((byte*)&num,sizeof(num)); // sizeof(num) == 4
}
...
mysocket_status tcp_mysocket::read_string(string &msg, long milisec) const
{
  ...
  iResult = read((byte*)buffer,len,milisec);
  if (iResult==mysocket_status::SOCK_ERROR ||
      iResult==mysocket_status::SOCK_TIMEOUT ||
      iResult==mysocket_status::SOCK_DISCONNECTED)
  {
...
mysocket_status tcp_mysocket::write_string(const string &msg) const
{
  ...
  iResult = write((byte*)msg.c_str(),len);
  if ( iResult == mysocket_status::SOCK_ERROR )
  {
    return mysocket_status::SOCK_ERROR;
  }
...
mysocket_status mysocket_queue::wait_read(long milisec)
{
  ...
  if (milisec >= 0) {
    ...
    intResult = ::select(nfds, &set, NULL, NULL, &t);
  }
  else {
    intResult = ::select(nfds, &set, NULL, NULL, NULL);
  }
...
mysocket_status mysocket_queue::wait_write(long milisec)
{
  ...
  if (milisec >= 0) {
    ...
    t.tv_sec = milisec/1000;
    t.tv_usec = 1000*(milisec - 1000*t.tv_sec);
    intResult = ::select(nfds, NULL, &set, NULL, &t);
  }
  else {
    intResult = ::select(nfds, NULL, &set, NULL, NULL);
  }
  if (intResult<0) return mysocket_status::SOCK_ERROR;
  if (intResult==0) return mysocket_status::SOCK_TIMEOUT;
  return mysocket_status::SOCK_OK;
