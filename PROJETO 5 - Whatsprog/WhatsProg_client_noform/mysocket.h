...
public:
  ...
  /// Leh uma sequencia de bytes de um socket conectado
  ...
  mysocket_status read(byte* dado, size_t len, long milisec=-1) const;

  /// Escreve uma sequencia de bytes em um socket conectado
  /// Soh pode ser usado em socket para o qual tenha sido feito um "connect" antes
  /// Ou entao em um socket retornado pelo "accept" de um socket servidor
  /// Retorna:
  /// - mysocket_status::SOCK_OK, em caso de sucesso;
  /// - mysocket_status::SOCK_ERROR, em caso de erro
  mysocket_status write(const byte* dado, size_t len) const;
